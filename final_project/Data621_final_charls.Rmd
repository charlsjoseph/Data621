...
author: "Mary Anna Kivenson + Elina v2 + Charls"
date: "May 13, 2020"
...
Since the linear step wise model has slight skewness for residual, lets try creating another model which is a generalized linear model. 

```{r}
glm_math <- glm(G3~., train_mat, family = gaussian)
test_mat$G3_pred <- predict(glm_math, newdata = test_mat)
glm_math <- stepAIC(glm_math, direction = "both", trace = FALSE)
summary(glm_math)


MathModelMetrics <- data.frame(matrix(ncol = 3, nrow = 0) ,stringsAsFactors = FALSE)
rmse <- RMSE(as.integer(test_mat$G3_pred), test_mat$G3)
r2 <- R2(as.integer(test_mat$G3_pred), test_mat$G3)



MathModelMetrics <- rbind(MathModelMetrics, c('GLM Model' ,round(rmse,2), round(r2,2)), stringsAsFactors = FALSE)



glm_por <- glm(G3~., train_por, family = gaussian)
test_por$G3_pred <- predict(glm_math, newdata = test_por)
glm_por <- stepAIC(glm_por, direction = "both", trace = FALSE)
summary(glm_por)

PorModelMetrics <- data.frame(matrix(ncol = 3, nrow = 0) ,stringsAsFactors = FALSE)

rmse <-  RMSE(as.integer(test_por$G3_pred), test_por$G3)
r2 <- R2(as.integer(test_por$G3_pred), test_por$G3)
PorModelMetrics <- rbind(PorModelMetrics, c('GLM Model', round(rmse,2), round(r2,2)), stringsAsFactors = FALSE)
...
r2 <- R2(mat_pred, test_mat$G3)
rmse <- RMSE(as.integer(mat_pred), test_mat$G3)
MathModelMetrics <- rbind(MathModelMetrics, c(' Linear Model' ,round(rmse,2), round(r2,2)), stringsAsFactors = FALSE)


r2 <- R2(por_pred, test_por$G3)
rmse <- RMSE(as.integer(por_pred), test_por$G3)
PorModelMetrics <- rbind(PorModelMetrics, c('Linear Model', round(rmse,2), round(r2,2)), stringsAsFactors = FALSE)
...
Lets compare the R-square and the RMSE of both models for Maths and portuguese class score. From the results, the linear model has good r2 and rmse score. 


```{r}

metrics <- c("Model", "RMSE", "R2")
colnames(MathModelMetrics) <- metrics
colnames(PorModelMetrics) <- metrics
```

### Math Pred( GLM-Gausian v/s Linear )

```{r}
kable(MathModelMetrics)
```

### portuguese Pred( GLM-Gausian v/s Linear )


```{r}
kable(PorModelMetrics)
