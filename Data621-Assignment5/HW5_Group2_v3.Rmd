...
author: 'Group 2: Elina Azrilyan, Charls Joseph, Mary Anna Kivenson, Sunny Mehta,
  Vinayak Patel'
...
df_eval <- read.csv("https://raw.githubusercontent.com/charlsjoseph/Data621/master/Data621-Assignment5/wine-evaluation-data.csv")[,-1] # drop the first column, it is the index

eval_1 <- subset(df_eval, select = -c(TARGET))
eval_1 <- eval_1 %>% encode_wine_data() %>% fill_wine_data()
...
```{r}
df_eval$TARGET <- predict(glm_fit, eval_1)

df_eval$TARGET <- round(df_eval$TARGET)
kable(df_eval)
```
